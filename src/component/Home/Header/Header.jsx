...
export default function Header({openForm}) {
  const {isDarkMode,toggleTheme,role}= useContext(ThemeContext)
  ...
  return (
    <Disclosure as="nav" className={`${isDarkMode ? "darkT": ""}  bg-gray-800 !bg-[#eee] z-20 sticky top-0 left-0 right-0`}>
  <div className="container mx-auto max-w-7xl px-2 sm:px-6 lg:px-8 h-20 pt-[5px] pb-[5px]">
    <div className="relative flex h-16 items-center justify-between">
      <div className="absolute inset-y-0 left-0 right-0 flex items-center  lg:hidden">
          ...
          {navigation.map((item) => (
                  <Link
                    ...
                    className={classNames(
                      location.pathname === item.href  ? 'bg-gray-900 text-white no-underline ' : ' !font-bold no-underline hover:bg-gray-700 hover:text-white',
                  isDarkMode ? "color": " " ,'rounded-md px-3 py-2 text-sm font-medium c'
                    )}
                  >
                    {item.name}
                  </Link>
      ...
      <div className="bg-transparent absolute inset-y-0 right-0 flex items-center sm:static sm:inset-auto sm:ml-6 ">
      <>
    <label className="themeSwitcherThree relative inline-flex cursor-pointer select-none items-center">
        ...
        />
        <div className="bg-transparent shadow-card flex h-[46px] w-[72px] items-center justify-center rounded-md ">
        <span
            className={`flex h-9 w-9 items-center text-white justify-center rounded ${
    ...
    </label>
{
  ...
  role === "admin"|| role === "user" || role === "instructor"  ?  <Profile/>:<button className={`${isDarkMode ? "color " : "text-[#495057]"} z-50 flex items-center gap-[5px] ml-[5px] hover:text-[18px] ease-in duration-200`} onClick={openForm}><span className='text-[14px] font-bold  '>Sign in</span> <FaUserAlt className=''/></button>
}
  ...
  <DisclosurePanel className="lg:hidden">
    ...
    {navigation.map((item) => (
            <Link
              ...
              aria-current={item.current ? 'page' : undefined}
              className={classNames(
                location.pathname===item.href ? 'bg-gray-900 text-white no-underline ' : 'no-underline text-black !font-bold hover:bg-gray-700 hover:text-white',
                'block rounded-md px-3 py-2 text-base font-medium',`${isDarkMode ? "text-white" : ""}`
              )}
            >
              {item.name}
            </Link>
